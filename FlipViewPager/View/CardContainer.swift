...
struct CardContainer: View {
    ...
    var body: some View {
        let firstLeftCard = CardView(mainSide: $mainSideLeft, anchor: $anchorLeft, degree: $firstDegree, geometrySize: $geometrySize, cardModel: $cardModel)
            .background(GeometryReader { proxy in Color.clear.onAppear() { imageSize = proxy.size } })
        let firstRightCard = CardView(mainSide: $mainSideLeft, anchor: $anchorRight, degree: $firstDegree, geometrySize: $geometrySize, cardModel: $cardModel)
            .background(GeometryReader { proxy in Color.clear.onAppear() { imageSize = proxy.size } })
        
        let secondLeftCard = CardView(mainSide: $mainSideRight, anchor: $anchorLeft, degree: $secondDegree, geometrySize: $geometrySize, cardModel: $cardModel)
            .background(GeometryReader { proxy in Color.clear.onAppear() { imageSize = proxy.size } })
        let secondRightCard = CardView(mainSide: $mainSideRight, anchor: $anchorRight, degree: $secondDegree, geometrySize: $geometrySize, cardModel: $cardModel)
            .background(GeometryReader { proxy in Color.clear.onAppear() { imageSize = proxy.size } })
                ...
                HStack(alignment: .center, spacing: 0) {
                    ...
                    .gesture(
                        DragGesture()
                            ...
                            .onEnded { _ in
                                if self.firstOffset.height > 0 {
                                    firstIncrementAnimation()
                                } else if self.firstOffset.height < 0{
